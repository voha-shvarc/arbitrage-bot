...
class SendAnalyticsService:
    ...
    HEADER_FIELDS = ["From", "To", "Pair", "Network", "To Buy",	"Avg Spread", "Base Profit", "Total Fee", "Profit", "Date"]
    ...
    def send_to_spreadsheet(self):
        ...
        self.sheets_service.values() \
            .append(spreadsheetId=self.spreadsheet_id, range=f"{self.sheet_name}!{self.cell_range}",
    ...
    def _set_header_row(self, sheet_name):
        self.sheets_service.values() \
            .append(spreadsheetId=self.spreadsheet_id, range=f"{sheet_name}",
                    valueInputOption="USER_ENTERED", body={'values': [self.HEADER_FIELDS]}) \
            .execute()
    ...
    def _get_rows(self):
        ...
        with Session() as session:
            to_sync_bundle_ids_subq = session.query(ProfitBundle.id) \
                .filter(ProfitBundle.status == BundleStatus.done,
                        ProfitBundle.synced.is_(False))

            bundles_qs = session.query(ProfitBundle) \
                .join(ProfitBundle.items) \
                .filter(ProfitBundle.id.in_(to_sync_bundle_ids_subq)) \
                .options(contains_eager(ProfitBundle.items),
                         ...
                         joinedload(ProfitBundle.base_exchange), joinedload(ProfitBundle.pair_exchange)) \
            ...
            for bundle in bundles_qs:
                ...
                for bundle_item in bundle.items:
                    ...
                    row = [bundle.base_exchange.name, bundle.pair_exchange.name, bundle.pair.default_name,
                           bundle.coin_network_exchange.network.name, round(bundle_item.to_use_usdt, 3),
                           f"{round(bundle_item.avg_spread * 100, 3)}%", round(bundle_item.base_profit, 3),
                           round(bundle_item.total_fee, 3), round(bundle_item.profit, 3), created_date]
                    rows.append(row)
