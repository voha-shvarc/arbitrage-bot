...
class Request:
    ...
    def __call__(self, method, path, auth=False, params={}, body={}):
        ...
        response = requests.request(method, url, headers=headers, timeout=self._timeout_sec, params=params, data=body)
        try:
            ...
            raise RuntimeError(f"text - {response.text}")

        if response.status_code != 200:
            raise RequestError(response_json.get("code", None), response_json.get("message", response.text))
