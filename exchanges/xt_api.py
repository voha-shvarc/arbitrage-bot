...
class XTAPI(AbstractExchange):
    ...
    def get_trading_pairs(self) -> List[TradingPair]:
        ...
        trading_pairs = [
            ...
            if pair["quoteAsset"] == "usdt" and pair["status"] == "ONLINE" and pair["tradingEnabled"]
        ]
    ...
    def withdraw(self, cne: CoinNetworkExchange, deposit_address: DepositAddress) -> None:
        ...
        try:
            ...
            if data is None:
                ...
                msg = ERROR_MAPPING[error_response.get("mc")]
                self.logger.error(f"[xt] {msg}")
                raise WithdrawError(f"[xt] {msg}")

        except JSONDecodeError as e:
            self.logger.exception(e)
            raise WithdrawError(f"[xt] Couldn't parse response, {response.text}") from e
